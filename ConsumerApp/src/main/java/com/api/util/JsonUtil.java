...
public class JsonUtil {
	...
	/*
	 * public static void loadSchema() { ObjectMapper mapper = new ObjectMapper();
	 * mapper.configure(SerializationFeature.INDENT_OUTPUT, true);
	 * 
	 * //Read the json schema File initialFile = new File("./schema.json");
	 * 
	 * try { InputStream schema = new FileInputStream(initialFile); JsonNode
	 * schemaNode = mapper.readTree(schema); //Add the input.json as the schema to
	 * validate all inputs against jsonSchema = factory.getJsonSchema(schemaNode); }
	 * catch (Exception e) { logger.error("Error loading the json schema"); } }
	 */
	/**
